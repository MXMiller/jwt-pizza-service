...
class DB {
  ...
  async getUser(email, password) {
    ...
    try {
      ...
      if (!user || (password && !(await bcrypt.compare(password, user.password)))) {
        metrics.authFailed();
        logger.errLogHelper(404, 'unknown user');
    ...
    try {
      ...
      for (const item of order.items) {
        ...
        if(item.menuId == 1){
          ...
          metrics.pepperoniSold();
        } else if(item.menuId == 3){
          metrics.margaritaSold();
        } else if(item.menuId == 4){
          metrics.crustySold();
        } else if(item.menuId == 5){
          metrics.charredLeopardSold();
        }
      }
    ...
    try {
      for (const admin of franchise.admins) {
        ...
        if (adminUser.length == 0) {
          metrics.authFailed();
          logger.errLogHelper(404, 'unknown user for franchise admin');
  ...
  async deleteFranchise(franchiseId) {
    ...
    try {
      ...
      try {
        ...
        logger.errLogHelper(500, 'unable to delete franchise');
  ...
  async getID(connection, key, value, table) {
    ...
    logger.errLogHelper(404, 'No ID found');
